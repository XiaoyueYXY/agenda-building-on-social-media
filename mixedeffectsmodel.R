...
fixed_effects_model <- lm(attention_media ~ lag_attention_media + attention_organization + lag_attention_organization + 
                            controversy*attention_organization + scope*attention_organization + positive_consequences*attention_organization + negative_consequences*attention_organization + elite_persons*attention_organization + 
                            controversy*lag_attention_organization + scope*lag_attention_organization + positive_consequences*lag_attention_organization + negative_consequences*lag_attention_organization + elite_persons*lag_attention_organization + 
                            controversy + scope + positive_consequences + negative_consequences + elite_persons,
                          data = final_data)
# Calculate VIF
vif_values <- vif(fixed_effects_model)
print(vif_values)
...
# Model 4: Test the interaction effect of newsworthiness without the impact of  positive consequence for H2
final_data$newsworthiness2 = rowSums(final_data[,c("surprise","controversy", "scope", "negative_consequences", "elite_persons")])

fixed_effects_model <- lm(attention_media ~ lag_attention_media + attention_organization + lag_attention_organization +
                            attention_organization*newsworthiness2 + lag_attention_organization*newsworthiness2,
                          data = final_data)
# Calculate VIF
vif_values <- vif(fixed_effects_model)
print(vif_values)

fm4 <- lmer(attention_media ~ lag_attention_media + attention_organization + lag_attention_organization + attention_organization*newsworthiness2 + lag_attention_organization*newsworthiness2 + (1|topic) + (1|Date), final_data)
summary(fm4)
r.squaredGLMM(fm4)
performance::icc(fm4, by_group = TRUE)
plot_model(
  fm4,
  type = "pred",
  terms = c("lag_attention_organization", "newsworthiness2"),
  title = "",  # Remove the figure title
  axis.title = c("Lagged Attention Corporate Organization", "News Organization Attention"),  # Rename x and y axes
  legend.title = "Revised Newsworthiness"  # Name the legend
)
